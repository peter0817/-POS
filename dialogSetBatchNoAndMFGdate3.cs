...
public class dialogSetBatchNoAndMFGdate3 : Form
{
	...
	public dialogSetBatchNoAndMFGdate3(frmEditInventory fni, string GDSNO)
	{
		InitializeComponent();
		_fni = fni;
		_GDSNO = GDSNO;
	...
	private void dialogSetBatchNoAndMFGdate3_Load(object sender, EventArgs e)
	{
	...
	private void btn_back_Click(object sender, EventArgs e)
	{
		...
		if (MessageBox.Show("未設定批號/製造日期之管制農藥商品，不能執行＂原廠退回/過期退貨＂。確定放棄？", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
		{
	...
	private void btn_SaveMemberDataAndSelect_Click(object sender, EventArgs e)
	{
		...
		else if (!dateTimePicker0.Checked)
		{
			text += "管制農藥商品，需選擇製造日期\n";
		}
		if (!text.Equals(""))
		...
		AutoClosingMessageBox.Show("原廠退回/過期退貨數量將自動回報至防檢局");
		Close();
	}
